...
public class CreditController {
  ...
  @ResponseBody
  public Result addCreditRecord(
      HttpServletRequest request, @Valid @RequestBody CreditRecord creditRecord) {
    try {
      // 获取请求头中的token令牌
      String token = request.getHeader("token");
      // 根据token解析出uid;
      DecodedJWT decodedJWT = JWTUtil.getTokenInfo(token);
      String uid = decodedJWT.getClaim("uid").asString();
      if (creditRecordService.insertCreditRecord(
          creditRecord.getOperation(), uid, creditRecord.getWay(), creditRecord.getNum())) {
