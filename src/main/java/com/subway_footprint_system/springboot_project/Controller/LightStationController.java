...
public class LightStationController {
  ...
  @Autowired private LightedStationServiceImpl lightedStationService;
  ...
  @ResponseBody
  public Result addLightedStation(
      HttpServletRequest request, @Valid @RequestBody LightedStation lightedStation) {
    try {
      // 获取请求头中的token令牌
  ...
  @ResponseBody
  public Result deleteLightedStation(
      HttpServletRequest request, @Valid @RequestBody LightedStation lightedStation) {
    try {
      // 获取请求头中的token令牌
      String token = request.getHeader("token");
      ...
      String uid = decodedJWT.getClaim("uid").asString();
      lightedStation.setUid(uid);
      if (!lightedStationService.deleteLightedStation(
          lightedStation.getUid(), lightedStation.getPid())) {
  ...
  @ResponseBody
  public Result getLightedStation(
      HttpServletRequest request, @Valid @RequestBody LightedStation lightedStation) {
    try {
      // 获取请求头中的token令牌
      String token = request.getHeader("token");
      ...
      String uid = decodedJWT.getClaim("uid").asString();
      lightedStation.setUid(uid);
      LightedStation lightedStation1 =
          lightedStationService.getLightedStation(lightedStation.getUid(), lightedStation.getPid());
  ...
  public Result getUserLightedStations(HttpServletRequest request) {
    try {
      ...
      lightedStation.setUid(uid);
      List<LightedStation> list =
          lightedStationService.getUserLightedStations(lightedStation.getUid());
