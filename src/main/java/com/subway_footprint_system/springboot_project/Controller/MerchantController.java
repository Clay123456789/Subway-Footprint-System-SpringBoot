...
public class MerchantController {
  ...
  public Result login(@Valid @RequestBody MerchantVo merchantVo, BindingResult bindingResult)
    ...
    if (bindingResult.hasErrors()) {
      String message =
          String.format("登陆失败，详细信息[%s]。", bindingResult.getFieldError().getDefaultMessage());
      return ResultFactory.buildFailResult(message);
    }
    String mid = null;
  ...
  @ResponseBody
  public Result sendRegistEmail(
      @Valid @RequestBody MerchantVo merchantVo, HttpSession httpSession) {
    /*
  ...
  public Result updateMerchant(
      HttpServletRequest request, @Valid @RequestBody MerchantVo merchantVo) {
    try {
      // 获取请求头中的token令牌
      String token = request.getHeader("token");
      // 根据token解析出mid;
