...
public class TreasureDaoImpl implements ITreasureDao {
  ...
  public boolean insertTreasure(Treasure treasure) {
    try {
      ...
      int result =
          jdbcTemplate.update(
              "insert into treasure(tid,variety,content,credit,pid,fromdate,todate,status,uid,mid,uid2,getdate,message) values(?,?,?,?,?,?,?,?,?,?,?,?,?) ",
              treasure.getTid(),
              treasure.getVariety(),
              treasure.getContent(),
              treasure.getCredit(),
              treasure.getPid(),
              treasure.getFromdate(),
              treasure.getTodate(),
              treasure.getStatus(),
              treasure.getUid(),
              treasure.getMid(),
  ...
  public boolean updateTreasure(Treasure treasure) {
    try {
      ...
      int result =
          jdbcTemplate.update(
              "update treasure set variety=?,content=?,credit=?,pid=?,fromdate=?,todate=?,status=?,uid=?,mid=?,uid2=?,getdate=?,message=? where tid=?",
              treasure.getVariety(),
              treasure.getContent(),
              treasure.getCredit(),
              treasure.getPid(),
  ...
  public List<Treasure> getPositionTreasure(String pid) {
    ...
    try {
      ...
      list =
          jdbcTemplate.query(
              "select * from treasure where pid=? and status=0 order by fromdate ASC",
              rowMapper,
              pid);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public List<Treasure> getAllTreasure() {
    ...
    try {
      ...
      list =
          jdbcTemplate.query(
              "select * from treasure where status=0 order by fromdate ASC", rowMapper);
    } catch (Exception e) {
      e.printStackTrace();
      return null;
    }
  ...
  public List<Treasure> getUserTreasure(String uid2) {
    ...
    try {
      ...
      list =
          jdbcTemplate.query(
              "select * from treasure where uid2=? order by status ASC,getdate ASC",
              rowMapper,
              uid2);
    } catch (Exception e) {
      e.printStackTrace();
