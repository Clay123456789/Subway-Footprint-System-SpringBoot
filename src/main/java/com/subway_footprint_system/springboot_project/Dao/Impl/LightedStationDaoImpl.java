...
public class LightedStationDaoImpl implements ILightedStationDao {
  ...
  public boolean insertLightedStation(LightedStation lightedStation) {
    try {
      ...
      int result =
          jdbcTemplate.update(
              "insert into lightedstation (uid,pid,time,credit) values(?,?,?,?)",
              lightedStation.getUid(),
              lightedStation.getPid(),
              lightedStation.getTime(),
              lightedStation.getCredit());
      if (1 == result) {
        return true;
      }
    } catch (Exception e) {
  ...
  public boolean deleteLightedStation(String uid, String pid) {
    try {
      // 返回影响行数，为1即删除成功
      int result =
          jdbcTemplate.update("delete from lightedstation where uid=? and pid = ?", uid, pid);
      if (1 == result) {
        return true;
      }
    } catch (Exception e) {
  ...
  public LightedStation getLightedStation(String uid, String pid) {
    RowMapper<LightedStation> rowMapper =
        new BeanPropertyRowMapper<LightedStation>(LightedStation.class);
    Object object = null;
    ...
    try {
      object =
          jdbcTemplate.queryForObject(
              "select * from lightedstation where uid=? and pid = ?", rowMapper, uid, pid);
  ...
  public List<LightedStation> getUserLightedStations(String uid) {
    ...
    try {
      ...
      list =
          jdbcTemplate.query(
              "select * from lightedstation where uid=? order by time ASC", rowMapper, uid);
    } catch (Exception e) {
      e.printStackTrace();
      return null;
    }
