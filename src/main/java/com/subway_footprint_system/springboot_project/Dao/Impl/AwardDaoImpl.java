...
public class AwardDaoImpl implements IAwardDao {
  ...
  public boolean insertAward(Award award) {
    ...
    try {
      int result =
          jdbcTemplate.update(
              "insert into award(aid, variety, num, name, content, credit, fromdate, todate, mid) values(?,?,?,?,?,?,?,?,?)",
              award.getAid(),
              award.getVariety(),
              award.getNum(),
              award.getName(),
  ...
  public boolean updateAward(Award award) {
    ...
    int result =
        jdbcTemplate.update(
            "update award set variety=?, num=?, name=?, content=?, credit=?,todate=?,status=? where aid=? ",
            award.getVariety(),
            award.getNum(),
            award.getName(),
            award.getContent(),
  ...
  public Award getAward(String aid) {
    ...
    try {
      ...
      Object object =
          jdbcTemplate.queryForObject("select * from award where aid=?", rowMapper, aid);
      // 插入缓存中
