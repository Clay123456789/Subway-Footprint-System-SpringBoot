...
public class SubwayDaoImpl implements ISubwayDao {
  ...
  /** 添加一行数据： 直接添加到mysql数据库,将缓存中数据所在表删除（为了保证selectAll获取数据一致） */
  @Override
  public boolean insertSubway(Subway subway) {
    // 返回影响行数，为1即增加成功
    int result =
        jdbcTemplate.update(
            "insert into subway(sid,code,cn_name,cename,cpre,l_xmlattr,p) values(?,?,?,?,?,?,?)",
            subway.getSid(),
            subway.getCode(),
            subway.getCn_name(),
            subway.getCename(),
            subway.getCpre(),
            JSONObject.toJSONString(subway.getL_xmlattr()),
            JSONObject.toJSONString(subway.getP()));
    if (result > 0) {
      // 判断是否缓存存在
  ...
  /** 删除一行数据： 先删除mysql数据库，再将缓存的数据删除即可 */
  @Override
  ...
  /** 修改一行数据： 先修改mysql数据库，再将缓存的数据删除即可，不直接更新缓存，效率太低。 */
  @Override
  public boolean updateSubway(Subway subway) {
    // 返回影响行数，为1表示修改成功
    int result =
        jdbcTemplate.update(
            "update subway set code=?,cn_name=?,cename=?,cpre,l_xmlattr=?,p=? where sid=? ",
            subway.getCode(),
            subway.getCn_name(),
            subway.getCename(),
            subway.getCpre(),
  ...
  /** 获取一行数据： 如果缓存(redis)中存在，从缓存中获取信息 如果缓存不存在，从 mysql中获取信息，然后插入缓存 */
  @Override
  public Subway getSubway(String sid) {
    ...
    try {
      ...
      List<Map<String, Object>> pmaplist =
          (List<Map<String, Object>>) JSONArray.parse((String) map.get("p"));
      ...
      Subway subway =
          new Subway(
              ...
              map.get("cn_name").toString(),
              map.get("cename").toString(),
              map.get("cpre").toString(),
              l_xmlattrmap,
              pmaplist);
      // 插入缓存中
  ...
  public Map<String, Object> getAllSubways() {
    ...
    List<Map<String, Object>> list1 =
        jdbcTemplate.queryForList("select DISTINCT code from subway order by code ASC ");
    // 依次找到线路和站点信息，并添加map
    for (Map<String, Object> temp : list1) {
      ...
      List<Map<String, Object>> list2 =
          jdbcTemplate.queryForList(
              "select *  from subway where code=? order by sid ASC", temp.get("code"));
      // 构建map数组
      for (int j = 0; j < list2.size(); j++) {
        List<Map<String, Object>> pmaplist =
            (List<Map<String, Object>>) JSONArray.parse((String) list2.get(j).get("p"));
        Map<String, Object> l_xmlattrmap =
            JSONObject.parseObject((String) list2.get(j).get("l_xmlattr"));
  ...
  public Map<String, Object> getAllSubways(int code) {
    ...
    List<Map<String, Object>> list =
        jdbcTemplate.queryForList("select * from subway where code = ? order by sid ASC ", code);
    // 构建map数组
    List<Map<String, Object>> maplist = new ArrayList<Map<String, Object>>();
    for (int i = 0; i < list.size(); i++) {
      List<Map<String, Object>> pmaplist =
          (List<Map<String, Object>>) JSONArray.parse((String) list.get(i).get("p"));
      Map<String, Object> l_xmlattrmap =
          JSONObject.parseObject((String) list.get(i).get("l_xmlattr"));
      Map<String, Object> sonmap = new HashMap<String, Object>();
      List<Map<String, Object>> sonmap2 = new ArrayList<Map<String, Object>>();
      sonmap.put("p", pmaplist);
      sonmap2.add(l_xmlattrmap);
