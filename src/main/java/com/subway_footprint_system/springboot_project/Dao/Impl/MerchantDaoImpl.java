...
public class MerchantDaoImpl implements IMerchantDao {
  ...
  public boolean insertMerchant(Merchant merchant) {
    try {
      ...
      int result =
          jdbcTemplate.update(
              "insert into merchant(mid,account,name,password,email,tel,location,authentication,authenticated,time,info) values(?,?,?,?,?,?,?,?,?,?,?)",
              merchant.getMid(),
              merchant.getAccount(),
              merchant.getName(),
              encryptor.encrypt(merchant.getPassword()),
              merchant.getEmail(),
              merchant.getTel(),
  ...
  /** 修改商户基本信息： 先修改mysql数据库，再将缓存的数据删除即可，不直接更新缓存，效率太低。 */
  @Override
  public boolean updateMerchant(Merchant merchant) {
    // 返回影响行数，为1表示修改成功
    int result =
        jdbcTemplate.update(
            "update merchant set account=?,name=?,tel=?,location=?,info=? where mid=?",
            merchant.getAccount(),
            merchant.getName(),
            merchant.getTel(),
            merchant.getLocation(),
  ...
  public boolean updateAuthentication(Merchant merchant) {
    ...
    int result =
        jdbcTemplate.update(
            "update merchant set authentication=?,authenticated=?,time=? where mid=?",
            merchant.getAuthentication(),
            merchant.getAuthenticated(),
            merchant.getTime(),
            merchant.getMid());
    if (result > 0) {
      // 判断是否缓存存在
  ...
  public boolean changePassword(String mid, String password) {
    ...
    int result =
        jdbcTemplate.update(
            "update merchant set password=? where mid=?", encryptor.encrypt(password), mid);
    if (result > 0) {
      // 判断是否缓存存在
      String key1 = "merchant_" + mid;
      Boolean hasKey1 = redisTemplate.hasKey(key1);
  ...
  /** 通过mid获取一行数据： 如果缓存(redis)中存在，从缓存中获取信息 如果缓存不存在，从 mysql中获取信息，然后插入缓存 */
  @Override
  ...
  public Merchant getMerchantByEmail(String email) {
    ...
    try {
      object =
          jdbcTemplate.queryForObject("select * from merchant where email = ?", rowMapper, email);
    } catch (EmptyResultDataAccessException e1) {
      // 查询结果为空，返回null
      return null;
    }
    ...
    try {
      object =
          jdbcTemplate.queryForObject(
              "select * from merchant where account = ?", rowMapper, account);
    } catch (EmptyResultDataAccessException e1) {
      // 查询结果为空，返回null
      return null;
    }
  ...
  public List<Merchant> getAllUnAuthenticatedMerchants() {
    ...
    try {
      ...
      list =
          jdbcTemplate.query(
              "select * from merchant where merchant.authenticated=0 order by time DESC",
              rowMapper);
    } catch (Exception e) {
      e.printStackTrace();
      return null;
