...
public class AwardRecordDaoImpl implements IAwardRecordDao {
  ...
  public boolean insertMysqlAwardRecord(AwardRecord awardRecord) {
    try {
      ...
      int result =
          jdbcTemplate.update(
              "insert into award_record (arid, operation, uid, mid, aid, num, time, credit) values(?,?,?,?,?,?,?,?)",
              awardRecord.getArid(),
              awardRecord.getOperation(),
              awardRecord.getUid(),
              awardRecord.getMid(),
              awardRecord.getAid(),
              awardRecord.getNum(),
              awardRecord.getTime(),
              awardRecord.getCredit());
      if (1 == result) {
        return true;
      }
    } catch (Exception e) {
  ...
  public boolean updateMysqlAwardRecord(AwardRecord awardRecord) {
    ...
    int result =
        jdbcTemplate.update(
            "update award_record set operation=?,num=?,credit=? where arid=?",
            awardRecord.getOperation(),
            awardRecord.getNum(),
            awardRecord.getCredit(),
            awardRecord.getArid());
