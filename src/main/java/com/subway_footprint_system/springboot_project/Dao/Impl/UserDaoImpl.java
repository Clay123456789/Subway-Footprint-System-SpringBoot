...
public class UserDaoImpl implements IUserDao {
  ...
  public boolean insertUser(User user) {
    try {
      ...
      int result =
          jdbcTemplate.update(
              "insert into user(uid,username,password,email,age,sex,tel,touxiang,qianming,credit) values(?,?,?,?,?,?,?,?,?,?)",
              user.getUid(),
              user.getUsername(),
  ...
  public boolean updateUser(User user) {
    ...
    int result =
        jdbcTemplate.update(
            "update user set username=?,age=?,sex=?,tel=?,touxiang=?,qianming=?,credit=? where uid=?",
            user.getUsername(),
            user.getAge(),
            user.getSex(),
            user.getTel(),
  ...
  public boolean changePassword(String uid, String password) {
    ...
    int result =
        jdbcTemplate.update(
            "update user set password=? where uid=?", encryptor.encrypt(password), uid);
    if (result > 0) {
      // 判断是否缓存存在
      String key1 = "user_" + uid;
      Boolean hasKey1 = redisTemplate.hasKey(key1);
  ...
  /** 通过uid获取一行数据： 如果缓存(redis)中存在，从缓存中获取信息 如果缓存不存在，从 mysql中获取信息，然后插入缓存 */
  @Override
  ...
  public User getUserByUsername(String username) {
    ...
    try {
      object =
          jdbcTemplate.queryForObject("select * from user where username = ?", rowMapper, username);
    } catch (EmptyResultDataAccessException e1) {
      // 查询结果为空，返回null
      return null;
    }
  ...
  public int getPersonalCreditRank(String uid) {
    int credit = 0;
    String sql =
        "SELECT obj_new.rownum FROM(SELECT obj.uid,obj.credit,"
