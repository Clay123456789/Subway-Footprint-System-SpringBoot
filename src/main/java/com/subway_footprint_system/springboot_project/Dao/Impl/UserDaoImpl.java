...
public class UserDaoImpl implements IUserDao {
    ...
    public boolean insert(User user) {
        //返回影响行数，为1即增加成功
        int result = jdbcTemplate.update("insert into User(UID,Account,Password,StationTable) values(?,?,?,?)",
                user.getUID(),user.getAccount(),user.getPassword(),user.getStationTable());
        if(result>0){
            // 判断是否缓存存在
    ...
    public boolean delete(User user) {
        int result = jdbcTemplate.update("delete from User where UID = ?",user.getUID());
        if(result!=0){
            // 判断是否缓存存在
    ...
    public boolean update(User user) {
        ...
        //返回影响行数，为1表示修改成功
        int result = jdbcTemplate.update("update User set Account=?,Password=?,StationTable=? where UID=?"
                ,user.getAccount(),user.getPassword(),user.getStationTable(),user.getUID());
        if(result > 0){
            // 判断是否缓存存在
            String key = "User_" + user.getUID();
            Boolean hasKey = redisTemplate.hasKey(key);
    ...
    public User select(User user) {
        ...
        try {
            object = jdbcTemplate.queryForObject("select * from User where UID = ?",rowMapper,user.getUID());
    ...
    public List<User> selectAll(User user) {
        ...
        //缓存中不存在
        RowMapper<User> rowMapper = new BeanPropertyRowMapper<User>(User.class);
        List<User> list = jdbcTemplate.query("select * from User order by UID DESC ",rowMapper);
