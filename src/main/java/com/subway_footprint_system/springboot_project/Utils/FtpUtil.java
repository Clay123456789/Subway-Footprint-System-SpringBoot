...
public class FtpUtil {
  ...
  /** 上传文件 */
  private boolean uploadlocalFile(String remoteFile, File localFile, long remoteSize)
      throws IOException {
   ...
   *
   * */
  private void deleteCompressImage(String remotePath, String fileName, double scale)
      throws IOException {
    if (!fileName.contains(SUFFIX)) {
      // 列出ftp服务器上的文件
      FTPFile[] ftpFiles = mFTPClient.listFiles(remotePath);
   ...
   * 以指定比率压缩ftp上的图片
   * */
  public List<String> compressImages(double scale, String remotePath, List<String> fileNames)
      throws IOException {
    List<String> urllist = new ArrayList<>();
    // 进入被动模式
    mFTPClient.enterLocalPassiveMode();
        ...
        if (i == ftpFiles.length) { // 略缩图不存在
          for (FTPFile ftpFile : ftpFiles) {
            if (ftpFile.getName().endsWith(fileName)) { // 找到原图
              if (!ftpFile.getName().contains(SUFFIX)
                  && isImage(getFileExtention(ftpFile.getName()))) {
