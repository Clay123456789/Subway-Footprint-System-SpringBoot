...
public class SubwayServiceImpl implements ISubwayService {
  @Autowired private SubwayDaoImpl subwayDao;
  ...
  public boolean updateSubway(Subway subway) {
    if (null != subway.getSid()) return subwayDao.updateSubway(subway);
    return false;
  }
    ...
    for (int i = 0; i < cities.size(); i++) {
      ...
      String url2 =
          "https://api.map.baidu.com/?qt=subways&c="
              + code
              + "&format=json&ak=yZSTYLk9UUvs0ZqXqBbtTp8ViKk5vxLM&v=3.0&from=jsapi";
      ...
      if (!str.equals("[]")) { // 存在部分城市暂无地铁信息，获取到的为空数组[]（不知为何？）
        ...
        if (object != null) {
          ...
          for (int j = 0; j < l.size(); j++) {
            ...
            Map<String, Object> l_xmlattr =
                JSONObject.parseObject(jsonObject6.getString("l_xmlattr"));

            // List<Map<String,Object>> p=
            JSONArray t = JSONObject.parseArray(jsonObject6.getString("p"));

            List<Map<String, Object>> p =
                (List<Map<String, Object>>) JSONObject.parse(t.toJSONString());

            String lid =
                JSONObject.parseObject(jsonObject6.getString("l_xmlattr")).getString("lid");

            if (subwayDao.getSubway(code + "_" + lid) == null) {
              if (!subwayDao.insertSubway(
                  new Subway(
                      code + "_" + lid,
