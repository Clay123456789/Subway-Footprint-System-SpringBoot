...
public class MerchantServiceImpl implements IMerchantService {
  ...
  public boolean updateMerchant(MerchantVo merchantVo) {
    if (null != merchantVo.getMid()) return merchantDao.updateMerchant(merchantVo);
    return false;
  }
  ...
  public boolean checkAuthentication(String mid) throws ParseException {
    ...
    if (null != merchant && 1 == merchant.getAuthenticated()) {
      ...
      long time =
          formatter.parse(JWTUtil.getNowTime()).getTime()
              - formatter.parse(merchant.getTime()).getTime();
