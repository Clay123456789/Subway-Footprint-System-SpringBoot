...
public class EMailServiceImpl implements IEMailService {
  @Autowired private JavaMailSender mailSender; // 一定要用@Autowired

  @Autowired private UserServiceImpl userService;

  @Autowired private MerchantServiceImpl merchantService;

  @Autowired private HttpSession httpSession_user, httpSession_merchant;
  ...
  public boolean registered_user(UserVo userVo) {
    ...
    if (null != userService.getUserByEmail(email)
        || null != userService.getUserByUsername(userVo.getUsername())) {
      return false;
    }
    // 将数据写入数据库
  ...
  public boolean registered_merchant(MerchantVo merchantVo) {
    ...
    // 如果用户名或邮箱已存在，注册失败
    if (null != merchantService.getMerchantByEmail(email)
        || null != merchantService.getMerchantByAccount(merchantVo.getAccount())) {
      return false;
    }
    // 将数据写入数据库
