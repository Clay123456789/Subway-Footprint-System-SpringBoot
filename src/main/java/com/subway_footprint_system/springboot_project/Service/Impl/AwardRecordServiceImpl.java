...
public class AwardRecordServiceImpl implements IAwardRecordService {
  @Autowired private AwardRecordDaoImpl awardRecordDao;
  @Autowired private CreditRecordServiceImpl creditRecordService;
  @Autowired private UserDaoImpl userDao;
  @Autowired private AwardDaoImpl awardDao;
  ...
  public boolean addUserBuryAwardRecord(String aid, String uid, int num, int credit) {
    ...
    AwardRecord awardRecord =
        new AwardRecord(uid + "-" + aid + "-" + time, 0, uid, mid, aid, num, time, credit);
    // award数量需相应减少
  ...
  public boolean insertShoppingAwardRecord(String aid, String uid, int num) {
    ...
    AwardRecord awardRecord =
        new AwardRecord(uid + "-" + aid + "-" + time, -1, uid, mid, aid, num, time, credit);
    return awardRecordDao.insertMysqlAwardRecord(awardRecord);
  }
  ...
  public boolean deleteOrderAwardRecord(String arid) {
    ...
    // 只能删除状态为1,3,4的订单
    if (null != awardRecord
        && (1 == awardRecord.getOperation()
            || 3 == awardRecord.getOperation()
            || 4 == awardRecord.getOperation())) {
      return awardRecordDao.deleteMysqlAwardRecord(arid);
    }
    return false;
  }
  ...
  public boolean createOrderAwardRecord(String aid, String uid, int num) {
    ...
    int credit = num * award.getCredit();
    AwardRecord awardRecord =
        new AwardRecord(uid + "-" + aid + "-" + time, 2, uid, mid, aid, num, time, credit);
    award.setNum(award.getNum() - awardRecord.getNum());
    // 若award剩余数量为0，状态改为已售空（2）
    if (0 == award.getNum()) {
      award.setStatus(2);
    ...
    return awardDao.updateAward(award)
        && awardRecordDao.insertMysqlAwardRecord(awardRecord)
        && awardRecordDao.insertRedisAwardRecord(awardRecord);
  }
  ...
  public boolean finishOrderAwardRecord(String arid) {
    ...
    // 用户扣除碳积分
    if (!creditRecordService.insertReduceCredit(
        awardRecord.getUid(), "兑换奖品", awardRecord.getNum())) {
      // 用户碳积分余额不足，订单结算失败
      return false;
    }
    // 更改数据库状态,从“订单中”变已兑换
